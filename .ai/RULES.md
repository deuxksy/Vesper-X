...
- `src/vesper_x/models_db.py` — `ModelRegistry`: data/models.db 이름 사전(변형→캐노니컬/slug). DB 부재 시 조용히 비활성
- `scripts/build_models_db.py` + `data/*.tsv` — 전수조사 스냅샷(TSV, git 추적)에서 models.db 재생성(DB는 gitignore 재생산물)
...
- **dispatch_log**(models.db) — crawl이 이미 전송한 post URL을 skip한다("신작만" 재크롤). 과거 이력이 없어 첫 크롤은 전량 받는다; 보유 분류가 필요한 신작만은 extract-only → heritage 보유 diff → delta dispatch
...
- `data/models.db`는 gitignore 재생산물(`uv run python scripts/build_models_db.py`로 생성) — 부재 시 `ModelRegistry`는 조용히 비활성이므로 신규 클론에서 metadata.models가 캐노니컬명이 아닌 사이트 표기로 기록되는 것은 정상 동작이다. 변형 매칭에서 `cosplayer` 등 일반명사 토큰은 제외(stopword)된다
